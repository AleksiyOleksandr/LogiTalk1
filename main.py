# Частина 1 — Імпорти та створення головного вікна
...
# Частина 2 — Елементи меню та теми
        self.label = CTkLabel(self.frame, text = 'Ваше Імʼя')
...
# Частина 3 — Поля чату, введення повідомлення, підключення до сервера
        ...
        try:
            ...
            self.sock.connect(('192.168.1.110', 8080))
            hello = (f"TEXT@{self.username}@[SYSTEM]{self.username} приєднався(лась) до чату!\n")
            self.sock.send(hello.encode('utf-8'))
            threading.Thread(target=self.recv_message, daemon=True).start()
        except Exception as e:
            self.add_message(f"Невдалось підключитися до сервера: {e}")

        self.adaptive_ui()

# Частина 4 — Надсилання, отримання та обробка повідомлень
...
# Частина 5 — Обробка рядків, зміна теми, відкриття/закриття меню
...
# Частина 6 — Адаптивність інтерфейсу, анімації меню, запуск програми
